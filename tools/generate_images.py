...
def footer(draw: ImageDraw.ImageDraw) -> None:
    rule(draw, 1846, w=2)
    draw.text((64, 1874), "董达｜AI 解决方案顾问｜电话 / 微信：18761576008", font=font(24), fill=BLACK)
...
def poster_profile() -> None:
    ...
    draw_header(
        ...
        "PROFILE 01",
        "解决方案顾问\n原型验证与落地",
        "把客户对 AI 的想象，变成一次可以下注的判断。",
    )
    metric_row(draw, 448, [("背景", "16 年"), ("方法", "先判断"), ("结果", "可验证")])

    draw.text((64, 664), "能力结构", font=font(44, True), fill=BLACK)
    rule(draw, 726)
    rows = [
        ("01", "听懂原话", "理解客户说了什么，以及真正担心的是什么。"),
        ("02", "重构问题", "找到更值得投入的改变，而不是照抄功能。"),
        ("03", "提出新切口", "给客户一个能够描绘未来的答案。"),
        ("04", "做出 Demo", "把关键判断做成可以点、可以试、可以讨论的对象。"),
        ("05", "守住落地", "把接口、异常、回滚与验收提前纳入方案。"),
    ]
    y = 760
    ...
    three_columns(
        ...
        1432,
        [
            ("SimpleHmi_WEILI", "交付规范：生产链路禁止 Mock，完成必须有可检验的证据。"),
            ("数字月台", "1 人 + AI 交付 12 台 HMI 协同系统，全网回滚 < 90 秒。"),
            ("机器人产线", "独占读卡 + 串行任务队列，稳定跑通设备协同全链路。"),
        ],
        h=286,
    )
    footer(draw)
...
def poster_task013() -> None:
    ...
    for i, item in enumerate(["Node / Fastify", "WebSocket 事件总线", "业务状态持久化", "部署控制器 / HMI Agent"]):
    ...
    text_box(
        draw,
        (92, 1718),
        "12 台 HMI 协同系统已上线；任一节点失败全网回滚 < 90 秒，上线至今现场事故 0 起。",
        font(24),
        880,
        WHITE,
        gap=8,
...
def hero_image() -> None:
    ...
    draw.text((120, 1190), "董达｜AI 解决方案顾问", font=font(42, True), fill=BLACK)
